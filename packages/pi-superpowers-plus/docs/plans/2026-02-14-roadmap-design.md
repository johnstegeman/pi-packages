# Roadmap Design — 2026-02-14

## Context

Brainstormed a full roadmap for pi-superpowers-plus. The package is at v0.2.0-alpha.1 with real users (no issues filed yet). Organized as versioned milestones with themed groupings within each.

## Decisions Made

- **Structure:** Versioned milestones (v0.2.0, v0.3.0, v0.4.0) with themed items within each
- **Logging:** File-based (`~/.pi/logs/superpowers-plus.log`), always-on at info level, verbose behind `PI_SUPERPOWERS_DEBUG=1`
- **Error handling:** Combined with logging — one pass through all catch blocks
- **CI:** Tests (`vitest`) + lint/types (`biome` + `tsc --noEmit`) + npm publish on git tags
- **Linting:** Biome (fast, zero-config, handles formatting too)
- **Publish:** Personal npm account, tag-triggered
- **Security:** Three targeted fixes — spawn sanitization, env filtering, path check tightening
- **Session persistence:** File-based keyed by git branch (`~/.pi/superpowers-plus/<branch>.json`), cross-session continuity is the primary goal, restart survival comes for free
- **Decision log:** Future item — builds on logging + session persistence

## Roadmap Content

The following is the full ROADMAP.md content to replace the current file:
