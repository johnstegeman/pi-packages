...
**DONE:** Generate the review package (`scripts/review-package <implement-step-id> BASE HEAD`, from this skill's directory — it prints the unique file path it wrote; BASE is the commit you recorded before dispatching the implementer — never the parent-commit base, which silently drops all but the last commit of a multi-commit task), then dispatch the task reviewer with the printed path.
...
- Hand the reviewer its diff as a file: run this skill's
  ...
  never the parent-commit base, which silently truncates multi-commit tasks. Never
