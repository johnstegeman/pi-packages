...
4. Process the returned envelope per task: `done` + `spec.compliant` + no critical/important → ledger `Task <N>: complete (...)`, close the bead. Non-compliant/issues → the issues become the open findings; run the EXISTING controller-side fix loop (declared `Gate:` → `fix-loop.js`; else the prose path; rounds 1–5 + breaker as normal — these are plain-Agent dispatches, so per-task cost events return here). `needs_context`/`blocked` → answer via `beads_comment`, re-dispatch on the next wave. `gate-failed`/`failed` → fix loop with the declared gate (or prose path). `review-failed` → re-run that review once on the plain path. Wave-level `degraded` → ledger note per task; nothing silently dropped.
