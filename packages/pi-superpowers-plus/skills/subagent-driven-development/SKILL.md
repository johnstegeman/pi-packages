...
4. Process the returned envelope per task: `done` + `spec.specCompliant` + no critical/important → ledger `Task <N>: complete (...)`, close the bead. `done_with_concerns` → ledger the concerns; the review proceeds and the task flows through the same complete/fix-loop branches as `done`. `specCompliant: false` or issues present → the issues become the open findings; run the EXISTING controller-side fix loop (declared `Gate:` → `fix-loop.js`; else the prose path; rounds 1–5 + breaker as normal — these are plain-Agent dispatches, so per-task cost events return here). `needs_context`/`blocked` → answer via `beads_comment`, re-dispatch on the next wave (the task bead stays claimed). `gate-failed`/`failed` → fix loop with the declared gate (or prose path). `review-failed` → re-run that review once on the plain path. Wave-level `degraded` → ledger note per task; nothing silently dropped.
...
Keep the resumed agent's `handle:` (recorded at dispatch) in the ledger, and append it to each fix-round line: after compaction, `get_subagent_result` by the handle retrieves the finished implementer's outcome without re-dispatch.
