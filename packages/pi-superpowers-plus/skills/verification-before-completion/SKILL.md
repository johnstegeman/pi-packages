...
When verification begins, claim the molecule's `verify` step: `beads_update({ id: "<verify-step-id>", claim: true })`.
...
When all verification passes, close the `verify` step (`beads_close({ ids: "<verify-step-id>", reason: "verification passed" })`), which unblocks `smoke-test-approved`. That gate needs a human to actually run/confirm the smoke test before resolving it (`beads_gate_resolve({ id: "<smoke-test-approved-gate-id>" })`) — announce this to the user rather than resolving it yourself. Once resolved, claim and work `finish` (`beads_update({ id: "<finish-step-id>", claim: true })`), handing off to `/skill:finishing-a-development-branch` as today — tell the user to run `/finish` (or `/skill:finishing-a-development-branch`).
...
or mark it `blocked` (`beads_update({ id: "<verify-step-id>", status: "blocked" })` with a `beads_comment({ id: "<verify-step-id>", text: "<why>" })`
) if you're certain it can't proceed as-is.
